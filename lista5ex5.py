def get_sum(number):
    ...
    for index in range(len(number_str)):
        soma += int(number_str[index])
    return soma

def fir_last_equal(number):
    number_str = str(number)
...
def conseq_numbers_equal(number):
    number_str = str(number)
    count_equal = 0
...
lista_tel =     [[213752, 216732, 221063, 221545, 225583, 229133, 230648, 233222],
...
for row in range(len(lista_tel)):
    for column in range(len(lista_tel[0])):
        even_number = get_sum(lista_tel[row][column])
        conseq_numbers = conseq_numbers_equal(lista_tel[row][column])
        first_last_numbers = fir_last_equal(lista_tel[row][column])

        numero_valido = lista_tel[row][column]
        if even_number % 2 == 0 and not conseq_numbers and not first_last_numbers:
            lista_validos.append(numero_valido)
